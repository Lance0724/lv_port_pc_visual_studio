...
static lv_point_precise_t side_linePoint[] = { {0, 0}, {0, SCREEN_VER/2} };
static lv_point_precise_t left_arrowPoint[] = { {0, 0}, {SCREEN_VER / 20, 0},{SCREEN_VER / 20, SCREEN_VER/40} };
static lv_point_precise_t right_arrowPoint[] = { {0, SCREEN_VER / 40}, {0, 0},{SCREEN_VER / 20,0} };
static lv_point_precise_t short_linePoint[] = { {0, 0}, {SCREEN_VER / 40, 0} };
static lv_point_precise_t mid_linePoint[] = { {0, 0}, {SCREEN_VER / 20, 0} };
//static lv_point_precise_t long_linePoint[] = { {0, SCREEN_VER / 5 * tan(abs(45) * PI / 180)}, {SCREEN_VER / 5, SCREEN_VER / 5 * tan(abs(45) * PI / 180)} };
 ...
 * @param mid_y 
*/
static void canvas_fresh(bool isNg,double angle, uint32_t mid_x, uint32_t mid_y) {
    for (uint32_t x = 0; x < SCREEN_HOR; x++)
    {

        for (uint32_t y = 0; y < SCREEN_VER; y++)
        {
            if (!isNg) {
                if (x < mid_x) {
                    if (y < mid_y) {
                        lv_canvas_set_px(canvas, x, y, lv_color_hex(0x1353EA), LV_OPA_COVER);
                    }
                    else if (y >= mid_y && y < mid_y + (mid_x - x) * tan(angle)) {
                        lv_canvas_set_px(canvas, x, y, lv_color_hex(0x1353EA), LV_OPA_COVER);
                    }
                    else {
                        lv_canvas_set_px(canvas, x, y, lv_color_hex(0x7f6000), LV_OPA_COVER);
                    }
                }
                else {
...
*/
void init_sg(uint32_t xbias, uint32_t ybias, double deg) {

    // static lv_color_t cbuf[LV_CANVAS_BUF_SIZE_TRUE_COLOR(SCREEN_HOR, SCREEN_VER)];
    LV_DRAW_BUF_DEFINE(draw_buf_16bpp, SCREEN_HOR, SCREEN_VER, LV_COLOR_FORMAT_RGB565);
    ...
    uint32_t mid_x = SCREEN_HOR / 2 + xbias;
    uint32_t mid_y = SCREEN_VER / 2 + ybias;
...
void init_leftLineMark(int step,int shift_px, int bias) {
    for (int i = 0; i <= (SCREEN_VER / 2) / 20; i++) {
        ...
        if (((SCREEN_VER / 2) / 20 / 2 + bias - i) * step >= 0) {
            ...
            // lv_label_set_recolor(label, true);                      /*Enable re-coloring by commands in the text*/ // Recolor is not supported for v9
...
//         //lv_label_set_recolor(left_label, true);                      // Recolor is not supported for v9
...
//         //lv_label_set_recolor(right_label, true);                      // Recolor is not supported for v9
