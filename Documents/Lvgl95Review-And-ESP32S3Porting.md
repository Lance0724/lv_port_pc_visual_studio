...
| 地平线出界 | 地平线完全离开视野后，不应再出现有限 card 的边缘 | 将 320×111 中间带投影到 Roll 对应的地平线法向；俯仰位移超过该范围时隐藏 card，只显示同侧 backdrop |
| 垂直俯仰边界 | ±90° 时地平线已完全离开视野，Roll 不应带回相反颜色 | 1.9 px/° 产生 171 px 法向位移，大于 320×111 中间带的半对角线；−90° 全地面，+90° 全天空 |
方案 A 仍使用旋转 card，但将背景从 320×200 扩展为 320×320，并把地平线移到
中间带中心。天地线运动直接沿用 `vhud()` 的方法：先按 1.9 px/° 算俯仰位移，再用
Roll 的整数正弦/余弦把位移旋转到地平线法向，最后同时写入
`transform_translate_x/y`。card 下方覆盖完整中间带的 backdrop 负责承接有限 card
外的区域，并按俯仰方向选择天空或地面色。代码同时计算当前 Roll 下中间带在法向的
投影；地平线已经完全出界时隐藏 card，避免 card 边缘与 backdrop 渐变形成斜向接缝。
