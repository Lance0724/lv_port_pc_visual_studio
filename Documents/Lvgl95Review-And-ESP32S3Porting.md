...
| 垂直俯仰边界 | ±90° 时地平线已完全离开视野，Roll 不应带回相反颜色 | `PITCH_VERTICAL_DDEG 900`；隐藏 card，−90° 显示全地面 backdrop，+90° 显示全天空 backdrop |
方案 A 仍使用旋转 card，但将背景从 320×200 扩展为 320×320，并把地平线移到
中间带中心。card 下方另有覆盖完整中间带的天空/地面 backdrop，用于普通姿态下
承接 card 边缘；达到 ±90° 时隐藏 card，绕开欧拉角垂直姿态的 Roll 奇点。读数速度
和高度统一使用 40 px 字体，避免高度达到四位时单独回退到 20 px；96 px 固定读数框
足以容纳四位数字。
