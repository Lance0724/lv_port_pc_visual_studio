...
| 梯线结构 | 覆盖 −90°..+90°，每 5° 一条；每 10° 标数字；20° 倍数长半宽 15 px、其余 10° 长半宽 16 px，5° 档短半宽 7/9 px；0° 由天地分界线表达；**无中缝** | `degree = -90..90` 循环 + `pitch_label()` 静态字符串 |
| 梯线数字位置 | ±(24..36)，字形描在梯线行 | `half + 14` 的 24 px 宽框，框顶上移 14 px |
...
| 天地线变换 | 俯仰位移必须始终垂直于滚转后的地平线 | 复用 `vhud()` 的整数向量算法：`tx = -pitch_px × sin(roll)`，`ty = pitch_px × cos(roll)` |
| 飞机视野型梯线 | 固定飞机中心必须指向当前 Pitch；Pitch 滚动方向始终垂直于梯线；Roll 与 Pitch 同时存在时必须复合 | 梯线绘制坐标先加 `pitch_px`，再由独立的 128×256 layer 围绕飞机中心应用 `transform_rotation(roll)`；当前 Pitch 对应的梯线恒过中心 |
| 地平线出界 | 地平线完全离开视野后，不应再出现有限 card 的边缘，但俯仰读数仍须可见 | 将 320×111 中间带投影到 Roll 对应的地平线法向；俯仰位移超过该范围时只隐藏 card 的 `sky`/`ground` 子对象，同侧 backdrop 承接背景，独立梯线继续显示 |
| 垂直俯仰边界 | ±90° 时地平线已完全离开视野，Roll 不应带回相反颜色，±90° 梯线仍应经过中心 | 1.9 px/° 产生 171 px 法向位移，大于 320×111 中间带的半对角线；−90° 全地面，+90° 全天空；梯线层不继承 card 的平移和裁剪 |
方案 A 仍使用 320×320 的旋转 card，并把地平线放在中间带中心。天地线运动直接沿用
`vhud()` 的方法：先按 1.9 px/° 算俯仰位移，再用 Roll 的整数正弦/余弦把位移旋转到
地平线法向，最后同时写入 `transform_translate_x/y`。card 下方覆盖完整中间带的
backdrop 负责承接有限 card 外的区域，并按俯仰方向选择天空或地面色。代码同时计算
当前 Roll 下中间带在法向的投影；地平线完全出界时只隐藏 card 的 `sky`/`ground`
填充，避免 card 边缘与 backdrop 渐变形成斜向接缝。

俯仰梯线采用飞机视野型：−90°..+90° 的刻线坐标先按当前 Pitch 沿未滚转的 Y 轴滚动，
然后独立的 128×256 layer 围绕固定飞机中心应用 Roll。等价结果是滚动方向始终垂直于
梯线，当前 Pitch 对应的刻线恒过飞机中心；在 Roll 与 Pitch 同时非零以及天地线已经
离开视野时，该关系仍保持。独立小 layer 避免继承平移后的 card 裁剪范围，但它仍是
额外的变换层，ESP32-S3 移植时须按 §2.5 实测其内存和刷新开销。

读数速度和高度统一使用 40 px 字体，避免高度达到四位时单独回退到 20 px；96 px
固定读数框足以容纳四位数字。
